...
def main():
  ...
  # Camera intrinsics from cam_intrinsic.py calibration results
  camera_k = np.array([(490.5992175, 0.0, 331.76523829), (0.0, 490.70353849, 245.05837429), (0.0, 0.0, 1.0)])
  camera_dist = np.array([(0.17352236, -0.28697994, -0.00061412, 0.00400239, 0.0244542)])

  # termination criteria, for aligning checkerboard corners onto an image
  criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)

  # Checkerboard shape: 8 rows × 11 cols of inner corners, 19 mm square size
  ...
  # Save per-scan selected LiDAR wall points and corresponding image paths for verify_extrinsic.py
  save_path = os.path.join(home, 'selected_lidar_wall_points.npy')
  np.save(save_path, np.array(laser_points, dtype=object), allow_pickle=True)
  image_paths = sorted([os.path.join(image_dir, f) for f in os.listdir(image_dir)])
  np.save(os.path.join(home, 'calibration_image_paths.npy'), np.array(image_paths), allow_pickle=True)
  print(f"Saved {len(laser_points)} per-scan wall point arrays to {save_path}")
  ...
  # Identity initialisation — ICP converges well without a prior here
